...
function diaryMatchesQuery(diary: DiaryEntry, query: string): boolean {
  ...
  return (
    ...
    (diary.mood ?? '').includes(q) ||
    diary.date.includes(q)
  )
}
...
export default function DiaryListPage() {
  ...
  const handleCardClick = useCallback(
    (id: string) => navigate(`/diary/${id}/edit`),
    [navigate],
  )
    ...
    <div className={styles.page}>
      ...
      <div className={styles.body}>
        <div className={styles.list}>
          ...
          {isHydrated && hasResults &&
            filteredDiaries.map((diary) => (
              <DiaryCard key={diary.id} diary={diary} onClick={handleCardClick} />
            ))}
        </div>
